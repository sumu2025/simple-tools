...
from simple_tools.core.batch_rename import (
    RenameConfig, RenameItem, RenameResult, BatchRenameTool, rename_cmd
)
...
class TestDataModels:
    ...
    def test_rename_config_model(self):
        """测试RenameConfig模型"""
        config = RenameConfig(
            pattern="old:new",
            path="/test/path",
        ...
        assert config.number_mode == False
        assert config.dry_run == True
        assert config.skip_confirm == False

    def test_rename_item_model(self):
        """测试RenameItem模型"""
        item = RenameItem(
    ...
    def test_rename_result_model(self):
        """测试RenameResult模型"""
        result = RenameResult(
            total=10,
            success=7,
...
class TestBatchRenameTool:
    ...
    def test_scan_files_with_filter(self, temp_dir):
        """测试带过滤器的文件扫描"""
        # 创建不同类型的文件
        (temp_dir / "file1.txt").write_text("content1")
        (temp_dir / "file2.txt").write_text("content2")
        ...
        # 只扫描txt文件
        config = RenameConfig(pattern="old:new", path=str(temp_dir), filter_pattern="*.txt")
        tool = BatchRenameTool(config)
        files = tool._scan_files()
    ...
    def test_parse_pattern_text_mode(self):
        """测试文本替换模式的模式解析"""
    ...
    def test_parse_pattern_number_mode(self):
        """测试序号模式的模式解析"""
    ...
    def test_parse_pattern_invalid_format(self):
        """测试无效模式格式"""
    ...
    def test_generate_new_name_text_mode(self, temp_dir):
        """测试文本替换模式的新名称生成"""
    ...
    def test_generate_new_name_text_mode_no_match(self, temp_dir):
        """测试文本替换模式无匹配的情况"""
    ...
    def test_generate_new_name_number_mode(self, temp_dir):
        """测试序号模式的新名称生成"""
    ...
    def test_check_conflicts_no_change(self, temp_dir):
        """测试文件名无变化的冲突检查"""
        file_path = temp_dir / "file.txt"
        file_path.write_text("content")
    ...
    def test_check_conflicts_file_exists(self, temp_dir):
        """测试目标文件已存在的冲突检查"""
        # 创建源文件和目标文件
    ...
    def test_execute_rename_success(self, temp_dir):
        """测试成功执行重命名"""
        # 创建测试文件
    ...
    def test_execute_rename_with_skipped(self, temp_dir):
        """测试包含跳过项的执行"""
        # 创建测试文件
        ...
        items = [
            RenameItem(
                old_path=file1,
                new_path=temp_dir / "renamed1.txt",
                status="pending"
            ),
            RenameItem(
    ...
    def test_run_text_replace_mode(self, temp_dir, cli_runner):
        """测试文本替换模式的完整流程"""
        # 创建测试文件
    ...
    def test_run_number_mode(self, temp_dir, cli_runner):
        """测试序号模式的完整流程"""
        # 创建测试文件
...
class TestCLICommand:
    ...
    def test_rename_command_basic(self, mock_confirm, temp_dir, cli_runner):
        """测试基本的rename命令（预览模式）"""
        # 创建测试文件
    ...
    @patch("click.confirm", return_value=True)
    def test_rename_command_number_mode(self, mock_confirm, temp_dir, cli_runner):
        """测试序号模式命令"""
        # 创建测试文件
        (temp_dir / "img1.jpg").write_text("image1")
        (temp_dir / "img2.jpg").write_text("image2")
    ...
    @patch("click.confirm", return_value=True)
    def test_rename_command_with_filter(self, mock_confirm, temp_dir, cli_runner):
        """测试带文件过滤的命令"""
        # 创建不同类型的文件
    ...
    def test_rename_command_execute_mode(self, temp_dir, cli_runner):
        """测试直接执行模式"""
        # 创建测试文件
        (temp_dir / "old_file.txt").write_text("content")
        ...
        result = cli_runner.invoke(
            rename_cmd,
            ["old:new", "-p", str(temp_dir), "--execute"]
        )
    ...
    def test_rename_command_skip_confirm(self, temp_dir, cli_runner):
        """测试跳过确认选项"""
        # 创建测试文件
        (temp_dir / "old_file.txt").write_text("content")
    ...
    def test_rename_command_no_matches(self, temp_dir, cli_runner):
        """测试没有匹配文件的情况"""
        # 创建不匹配的文件
        ...
        result = cli_runner.invoke(
            rename_cmd,
            ["old:new", "-p", str(temp_dir), "-f", "*.jpg"]
        )

        assert result.exit_code == 0
        assert "没有找到匹配的文件" in result.output

    def test_rename_command_invalid_pattern(self, cli_runner):
        """测试无效的重命名模式"""
        # 文本模式下没有使用冒号分隔
        result = cli_runner.invoke(
            rename_cmd,
    ...
    def test_rename_command_all_skipped(self, temp_dir, cli_runner):
        """测试所有文件都被跳过的情况"""
        # 创建不包含要替换文本的文件
    ...
    @patch("click.confirm", return_value=True)
    def test_rename_command_mixed_results(self, mock_confirm, temp_dir, cli_runner):
        """测试混合结果（部分成功、部分跳过）"""
        # 创建测试文件
        (temp_dir / "old_file1.txt").write_text("content1")
        (temp_dir / "old_file2.txt").write_text("content2")
    ...
    def test_rename_command_user_cancel(self, mock_confirm, temp_dir, cli_runner):
        """测试用户取消操作"""
        # 创建测试文件
        (temp_dir / "old_file.txt").write_text("content")
