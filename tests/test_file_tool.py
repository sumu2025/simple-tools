...
from simple_tools.core.file_tool import list_files, format_size, format_time, list_cmd
...
class TestListFiles:
    ...
    def test_list_files_with_details(self, temp_dir):
        """测试详细信息显示功能"""
        # 创建测试文件
    ...
    def test_list_files_sorting(self, temp_dir):
        """测试文件排序功能"""
        # 创建混合大小写的文件和目录
        ...
        # 验证目录在前
        assert result[0]["is_dir"] == True
        assert result[1]["is_dir"] == True
        assert result[2]["is_dir"] == False
    ...
    def test_list_file_not_directory(self, temp_dir):
        """测试路径不是目录的情况"""
        from click import ClickException
...
class TestListCommand:
    ...
    def test_list_command_basic(self, temp_dir, cli_runner):
        """测试基本的list命令"""
        # 创建测试文件
    ...
    def test_list_command_with_all_option(self, temp_dir, cli_runner):
        """测试带 --all 选项的命令"""
        # 创建隐藏文件
        (temp_dir / ".hidden").write_text("hidden")
        (temp_dir / "visible.txt").write_text("visible")
    ...
    def test_list_command_with_long_option(self, temp_dir, cli_runner):
        """测试带 --long 选项的命令"""
        # 创建测试文件
    ...
    def test_list_command_empty_directory(self, temp_dir, cli_runner):
        """测试空目录的命令输出"""
        mock_ctx = {"config": type('Config', (), {"verbose": False})()}
    ...
    def test_list_command_default_current_directory(self, cli_runner, monkeypatch):
        """测试默认使用当前目录"""
        mock_ctx = {"config": type('Config', (), {"verbose": False})()}
        ...
        def mock_list_files(directory, **kwargs):
            return [
