...
class TestTextReplaceTool:
    ...
    def test_replace_config_properties(self) -> None:
        """测试替换配置属性解析."""
    ...
    def test_scan_files_single_file(self, tmp_path: Path) -> None:
        """测试单文件扫描."""
        # 创建测试文件
        test_file = tmp_path / "test.txt"
        test_file.write_text("hello world")

        config = ReplaceConfig(pattern="hello:hi", file=str(test_file))
    ...
    def test_scan_files_directory(self, tmp_path: Path) -> None:
        """测试目录扫描."""
        # 创建测试文件
        ...
        config = ReplaceConfig(pattern="old:new", path=str(tmp_path))
    ...
    def test_scan_files_with_extension_filter(self, tmp_path: Path) -> None:
        """测试扩展名过滤."""
        # 创建不同扩展名的文件
        (tmp_path / "file1.txt").write_text("content1")
        (tmp_path / "file2.py").write_text("content2")
        ...
        config = ReplaceConfig(
            pattern="old:new", path=str(tmp_path), extensions=[".txt", ".md"]
        )
    ...
    def test_process_file_preview(self, tmp_path: Path) -> None:
        """测试文件预览处理."""
        # 创建测试文件
        test_file = tmp_path / "test.txt"
    ...
    def test_process_file_execute(self, tmp_path: Path) -> None:
        """测试文件执行替换."""
        # 创建测试文件
        test_file = tmp_path / "test.txt"
    ...
    def test_process_file_no_match(self, tmp_path: Path) -> None:
        """测试没有匹配的文件."""
        test_file = tmp_path / "test.txt"
    ...
    def test_process_file_encoding_error(self, tmp_path: Path) -> None:
        """测试编码错误处理."""
        # 创建二进制文件
    ...
    def test_empty_directory_handling(self, tmp_path: Path) -> None:
        """测试空目录处理."""
        config = ReplaceConfig(pattern="hello:hi", path=str(tmp_path))
