...
class TextReplaceTool:
    ...
    def __init__(self, config: ReplaceConfig):
        self.config = config
        logfire.info("初始化文本替换工具", attributes={
    ...
    def process_file(self, file_path: Path, execute: bool = False) -> ReplaceResult:
        ...
        try:
            ...
            if self.config.old_text in content:
                ...
                if execute:
                    # 执行替换
                    new_content = content.replace(self.config.old_text, self.config.new_text)
                    ...
                    for i, line in enumerate(lines, 1):
                        if self.config.old_text in line:
                            ...
                            new_line = f"       → {line.replace(self.config.old_text, self.config.new_text).strip()}"
    ...
    def run(self) -> None:
        """执行替换流程"""
        with logfire.span("text_replace", attributes={
            ...
            for result in matched_results:
                ...
                if exec_result.error:
                    ...
                    click.echo(f"  ✓ {result.file_path.name} - 成功替换 {result.match_count} 处")
                    success_count += 1
            ...
            logfire.info("文本替换完成", attributes={
...
@click.option("-f", "--file", type=click.Path(exists=True), help="指定单个文件")
@click.option("-p", "--path", default=".", help="指定扫描目录")
@click.option("-e", "--extension", multiple=True, help="文件扩展名过滤（可多次使用）")
@click.option("--execute", is_flag=True, help="执行实际替换（默认为预览模式）")
@click.option("-y", "--yes", is_flag=True, help="跳过确认提示")
@click.pass_context
def replace_cmd(ctx, pattern, file, path, extension, execute, yes):
    """
    文本批量替换工具
