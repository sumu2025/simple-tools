...
def list_files(directory, show_hidden=False, show_details=False):
    """
    列出指定目录下的文件和文件夹
    参数：
        directory - 目标目录路径字符串
        ...
        try:
            ...
            for item in items:
                ...
                if show_details and not is_dir:
                    ...
                    info["modified"] = os.path.getmtime(item_path)
                    info["modified_formatted"] = format_time(info["modified"])

                items_info.append(info)

            logfire.info(f"列出目录内容成功: {directory}", attributes={"item_count": len(items_info)})
...
@click.option("-a", "--all", is_flag=True, help="显示隐藏文件（以.开头的文件）")
@click.option("-l", "--long", is_flag=True, help="显示详细信息（文件大小、修改时间）")
@click.pass_context
def list_cmd(ctx, path, all, long):
    """
    列出指定目录下的文件和文件夹
    ...
    try:
        ...
        for item in items:
            ...
            if long and not item["is_dir"]:
                click.echo(f"{type_indicator:<6} {item['name']:<30} {size_info:<10} {item['modified_formatted']}")
            else:
